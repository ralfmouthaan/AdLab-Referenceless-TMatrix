...
Namespace TypeConversions
    Module modTypeConversions
        ...
        End Sub
        Public Sub RealToComplex(ByRef Input(,) As UInt16, ByRef Output As Numerics.Complex(,))
        ...
        End Sub
        Public Sub IntegerToDouble(ByVal Input(,) As Integer, ByRef Output As Double(,))

            ReDim Output(UBound(Input, 1), UBound(Input, 2))

            For i = 0 To UBound(Input, 1)
                For j = 0 To UBound(Input, 2)
                    Output(i, j) = CDbl(Input(i, j))
                Next
            Next

        End Sub
        Public Sub UInt16ToDouble(ByVal Input(,) As UInt16, ByRef Output As Double(,))
